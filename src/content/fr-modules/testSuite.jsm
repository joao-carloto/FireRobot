	...
	function runTest() {
		...
		//TODO add support for Linux and OSx.
		if (OSName != "Windows") {
			warning(null, "firerobot.warn.only-windows");
			return;
		}
	...
	function showReport() {
		...
		if (reportFile.exists()) {
			_windowWatcher.openWindow(null, "file:\\\\\\" + reportFile.path,
				"Fire Robot Report",
				"menubar,location,toolbar,resizable,scrollbars,status",
				null);
		} else {
